<template>
  <div class="auth-card">
    ...
    <!-- 注册表单 -->
    <form class="auth-form" @submit.prevent="handleResetPassword" novalidate>
      <!-- 昵称 -->
...
const sendVerificationCode = async () => {
  ...
  try {
    await AuthAPI.sendCaptcha(form.email, 'reset')
    alert('验证码已发送至邮箱，请查收')
  } catch (error: any) {
...
const handleResetPassword = async () => {
  ...
  try {
    ...
    alert('重置密码成功！即将跳转到登录页')
    setTimeout(() => {
      router.push('/login?registered=true')
    }, 1500)
  } catch (error: any) {
    registerError.value = error.response?.data?.message || '重置密码失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
